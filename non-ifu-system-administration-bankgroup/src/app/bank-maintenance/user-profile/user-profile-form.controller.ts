...
export class BGABankUserProfileFormController {
    ...
    constructor(
        ...
        if (this.formState === this.FormDisplayState.create) {
            BGABankUserProfileService.initiate().then(this.loadData);
        }
        else if (this.formState === this.FormDisplayState.update)  {
            BGABankUserProfileService.edit(this.$stateParams['userId']).then(this.loadData);
        }
        else if (this.formState === this.FormDisplayState.view) {
            BGABankUserProfileService.view(this.$stateParams['userId']).then(this.loadData);
        }
    }
    ...
    private create = (): void => {
        this.BGABankUserProfileService.create(this.formConfiguration.model)
                .then(
                    // Success
                    () => this.$state.go('app.bga.bank-maintenance.user-profile.list'),
                    // Error
    ...
    private update = (): void => {
        this.BGABankUserProfileService.update(this.formConfiguration.model)
                .then(
                    // Success
                    () => this.$state.go('app.bga.bank-maintenance.user-profile.list'),
                    // Error
